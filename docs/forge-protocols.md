...
2. 否则回退到全局 staging：`C:\Users\Administrator\.claude\vendors\bmad-method-staging`
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\install-bmad-local.ps1" -RepoPath "<repo>"
```
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\write-forge-routing.ps1" -RepoPath "<repo>" -Level L4 -ProjectLevel L4 -ParentLevel L4 -InheritedFrom "<parent-ref>" -ExecutionScope light -Mode full -Execution guided-full -PipelinePhase M1 -BatchProtocol build
```
...
初始化命令：`pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\Initialize-ForgeGuidedFullState.ps1" -RepoPath "<repo>" -Phase 1A -QuestionPending`。
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\Test-ForgeGuidedFullGate.ps1" -RepoPath "<repo>" -CandidateText "<assistant draft>"
```
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\write-forge-routing.ps1" -RepoPath "<repo>" -Level L3 -Mode full -Execution guided-full -AdoptionMode local_routing -Risk high -Cost medium -WriteScope cross_module -Reason shared_contract,security_auth
```
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\write-forge-routing.ps1" -RepoPath "<repo>" -Level L3 -Mode full -Execution guided-full -AdoptionMode local_routing -PipelinePhase M1 -TaskGroup M1.1 -GroupStatus started -WriteScope module -VerificationRef "<planned>" -Reason implementation_group
```
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\write-forge-routing.ps1" -RepoPath "<repo>" -Level L4 -Mode full -Execution guided-full -PipelinePhase M1 -TaskGroup W1-B -GroupStatus completed -HighRisk "auth,jwt,signature" -TodoRef "TodoWrite:W1-B" -TddRef "red-green:auth-jwt-init-data" -TestRef "unit:pass:14" -VerificationRef "typecheck:pass;unit:pass" -CommitSha "<sha>" -LearningsRef ".claude/compound-learnings.md" -NextPhase W2
```
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\Test-ForgeM1Compliance.ps1" -RepoPath "<repo>" -TaskGroup "M1.1" -CandidateText "<assistant draft>"
```
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\update-forge-state.ps1" -RepoPath "<repo>" -Mode fix -CurrentFocus "<focus>" -OpenRisks "<risk>" -NextActions "<next>" -Learning "<learning>"
```
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\forge single-entry adopt-natural.ps1" -RepoPath "<repo>" -Action audit
```
...
- Stop 审计日志 fallback：若 `<repo>/.claude/forge-drift-audit.jsonl` 因沙箱、权限或文件锁无法追加，audit 会重试并写 `forge-drift-audit-fallback.jsonl`；优先路径是 `C:\Users\Administrator\.claude\logs\forge-drift-audit-fallback.jsonl`，最后兜底 `%TEMP%`。
...
```powershell
# 检查指定任务组
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\Test-ForgeM1Compliance.ps1" -RepoPath "<repo>" -TaskGroup "M1.1" -CandidateText "<assistant draft>" -Json

# 检查最近一个 M1 任务组
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\Test-ForgeM1Compliance.ps1" -RepoPath "<repo>" -Latest -Json

# 检查所有 started 但未 completed 的 M1 任务组；没有 open group 时返回 ok=true，并带 note=no_open_m1_task_groups
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\Test-ForgeM1Compliance.ps1" -RepoPath "<repo>" -AllOpenGroups -Json
```
...
```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "C:\Users\Administrator\.claude\scripts\forge-smoke.ps1" -LiveClaudeRoute
```
...
- `Test-ForgeLiveRouteFreshness.ps1`：读取 `C:\Users\Administrator\.claude\logs\forge-smoke.jsonl`，确认最近 live route 覆盖 `quick/fix/build/full/ship` 与 `audit-only/auto/guided-full` execution、全部 `passed=true`、未超过 `-MaxAgeHours`，并可用 `-RequiredClaudeVersion` 绑定当前 Claude Code 版本。
